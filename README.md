...
另外还有两个变体项目，都复用本项目 `leadlag/` 里已经调好的统计核心：

- **`limitup/`**：把"leader 触发"的定义从"今天上涨"换成"今天收盘封涨停"
  （板块内龙头带动跟风股次日补涨的假设）。见 `limitup/README.md`。
- **`intraday/`**：`limitup` 的盘中版本——"龙头股某根 K 线首次触及涨停后，
  同板块跟随股在之后几根 K 线内（同一交易日）上涨"，验证的是盘中传导而不是
  隔夜传导，需要分钟线，挖掘/回测的对齐方式跟日线版本不一样。见
  `intraday/README.md`。
...
2. **样本外验证（更重要，且必须也做 FDR 校正）**：即使通过了 FDR 校正，单个
   配对仍然可能只是某一段历史窗口的偶然产物（比如两只票恰好在某个题材周期里
   一起被资金炒作）。`research/run_mining.py` 强制切出训练/测试两段时间，把
   训练窗口挖出来的候选配对拿到测试窗口上重新算一遍。**这里踩过一次真实的坑**：
   早期版本只检查"测试窗口 lift 是不是正的"，结果在全市场实测中候选池一大
   （比如用 `--candidate-mode top-n` 或放宽 `--alpha` 后有几十万候选），纯噪声
   配对也有约 50% 会"恰好"在测试窗口保持同号——这和抛硬币没区别，根本不是
   有效过滤。现在 `leadlag.factor.filter_oos_significant` 会对样本外 p 值
   **重新做一次 BH-FDR 校正**，而不是只看符号，这才是真正把关的一步（对应
   `run_mining.py` 里的 `--oos-alpha`，不要为了让配对"活下来"就调大它）。
