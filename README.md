...
另外还有一个**变体项目 `limitup/`**：把"leader 触发"的定义从"今天上涨"换成
"今天收盘封涨停"（板块内龙头带动跟风股补涨的假设），复用本项目 `leadlag/`
里已经调好的统计核心（挖掘/FDR/样本外验证/回测引擎完全共用，只是新增了非对称
版本以支持 leader 和 follower 用不同的信号定义）。为什么单独建这个项目、
它跟这里的关系是什么，见 `limitup/README.md`。
