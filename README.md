...
5. **共同因子污染的诊断信号：一个 follower 对应很多 leader**。如果某个 follower
   同时和十几个互不相关的 leader "显著"，它更可能只是对全池共同因子 beta 特别
   高（任何成分先动，它随后大概率也动），而不是真有十几个独立的逐对关系。
   `leadlag.factor.exclude_hub_followers` 把这类 follower 整个剔除，`run_etf_mining.py`
   默认 `--max-leaders-per-follower=3`。**被它过滤掉的比例本身就是一个诊断指标**
   ——在 `intraday/` 的 T0-ETF 实测里这个比例是 74%~97%，直接说明了池子越同质
   （全是跨境ETF），`--mode excess` 的截面中位数越剔不干净。
6. **先算"最小变动价位 ÷ 价格"，再去挖信号**。A 股最小报价变动是 0.001 元，
   对一只 1 块钱的 ETF 就是 10bp 的价格颗粒度；如果你要找的边际只有几个 bp，
   **利润比一个跳价单位还小，物理上就不可能被市价单捕获**，挖得再显著也没用。
   这一步几乎零成本，应该放在整个流程最前面。
7. **反解盈亏平衡成本，而不是争论滑点该填几**。跑两个成本点（比如滑点 0 和
   5bp），总收益对成本近似线性，直接解出"要盈利，总往返成本必须低于多少"，
   再拿这个数字跟现实对照。`run_etf_backtest.py` 的
   `--commission-bps`/`--slippage-bps`/`--stamp-tax-bps` 就是干这个的。
   （**ETF 免征印花税**，印花税只对个股卖出方征收——这个默认值本项目搞错过一次。）

### 一份完整的否定结论（值得一读）

`intraday/README.md` 的"最终结论"一节记录了一个假设（T0-ETF 日内涨幅阈值触发
→ 同日跟涨）被**七轮真实数据检验逐步否定**的全过程：从"挖到10个样本外FDR显著
配对"开始，到最后反解出"毛边际 1.4~3bp **比最便宜的佣金还小**"为止，每一轮的
具体数字都在。上面这7条防御每一条都在那个案例里真实地拦截过一次错误结论——
按其中任何一轮的中间结果上实盘，2.2年会亏掉三成。
